...
def predict(model,loc,ser,df):
    ...
    result = result.drop(['Location_encoder','Skillset_encoder',"index"], axis='columns')
    r1 = result["Name"].tolist()
    r2 = result["Contact no"].tolist()
    r3 = result["Skillset"].tolist()
    r4 = result["Location"].tolist()
    r5 = result["Experience"].tolist()
    r6 = result["Customer rating"].tolist()
...
def findnow(loc,ser):
    if (area[loc]):
        ...
        for j in range(len(dict)):
            ...
            lis = dict[j][0]
            result,r1,r2,r3,r4,r5,r6 = predict(model, lis, ser,df)
